...
```mermaid
flowchart LR
  subgraph LC[LeetCode tab]
    P[LeetCode page]
    INJ[Injected script\n(in-page fetch/XHR hook)]
    CS[Content script\n(orchestrator)]
    P --> INJ --> CS
  end
...
### Sequence diagram (Submit → Accepted → Sync)
...
sequenceDiagram
  ...
  participant GQL as LeetCode GraphQL
  participant BG as Background worker
  participant GH as GitHub API

  U->>LC: Click Submit
  LC->>INJ: POST /graphql (submit mutation with typed_code)
  INJ->>CS: SUBMIT_INTENT
  INJ->>CS: SUBMISSION_CAPTURED { submit, response }
  CS->>CS: Normalize (bundled schema mapping)
  CS->>GQL: questionData(titleSlug)
  CS->>GQL: submissionDetails(submissionId) [poll until final]
  CS->>CS: Validate Accepted-only
  CS->>BG: Store lastSubmission + update captureStatus
  alt Auto-push ON
    CS->>BG: PUSH_LAST
  else Auto-push OFF
    U->>BG: Click Sync now in popup
  end
  BG->>GH: Git Data API (blobs/tree/commit/ref) [single commit]
  BG->>BG: Clear lastSubmission; append recentSyncs
```
